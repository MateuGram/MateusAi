...
def chat():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"⏳ Модель загружается: {current_model}")
        else:
...
def test():
    ...
    return jsonify({
        ...
        "html_loaded": bool(HTML_TEMPLATE)
    })
...
if __name__ == '__main__':
    ...
    print(f"📄 HTML загружен: {bool(HTML_TEMPLATE)}")
    app.run(host='0.0.0.0', port=port, debug=False)@app.route('/chat', methods=['POST'])
def chat():
    """Обрабатывает сообщения пользователя и возвращает ответ от AI."""
    global current_model
    data = request.json
